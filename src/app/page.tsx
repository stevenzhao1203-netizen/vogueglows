...
export default function HomePage() {
  return <main>
    ...
    <section className="home-hero"><div><p>VogueGlows Journal</p><h1>Things to wear, use, and come back to.</h1><span>Notes on getting dressed, traveling lighter, and choosing the objects that make ordinary days run more smoothly.</span><a href="/journal">Read the journal <ArrowRight size={16} /></a></div><img src="/images/vg-hero-still-life.webp" alt="Sunlit gallery still life with a black tote and ivory linen" fetchPriority="high" decoding="async" /></section>
    ...
    <section className="product-section" id="latest"><div className="section-heading"><p>The Edit</p><h2>A short list with the details that matter.</h2><span>Shopping notes for pieces worth a closer look. A merchant link appears only after we have checked the listing and where it leads.</span></div><div className="product-grid product-grid-text">{products.map((product) => <article className="product-card" key={product.slug}><p>{categories.find((category) => category.slug === product.category)?.name}</p><h3><a href={`/products/${product.slug}`}>{product.name}</a></h3><span>{product.summary}</span><a className="text-link" href={`/products/${product.slug}`}>Open guide <ArrowRight size={15} /></a></article>)}</div><div className="edit-link"><a href="/trends">Browse The Edit <ArrowRight size={15} /></a></div></section>
    <SiteFooter />
  </main>;
}
