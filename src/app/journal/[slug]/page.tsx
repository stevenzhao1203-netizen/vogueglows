...
export default async function ArticlePage({ params }: { params: Promise<{ slug: string }> }) {
  ...
  const relatedProducts = products
    .filter((product) => categories.find((category) => category.slug === product.category)?.name === article.category)
    .slice(0, 3);
    ...
    <main>
      ...
      <article className="article article-post">
        <header className="article-header">
          <p>Journal / {article.category}</p>
          <h1>{article.title}</h1>
          <span>{article.excerpt}</span>
        </header>
        <figure className="article-figure">
          <img src={article.image} alt="" decoding="async" />
          <figcaption>A note on {article.category.toLowerCase()} and the details worth checking in real life.</figcaption>
        </figure>
        <nav className="toc" aria-label="In this article">
          <p>In this note</p>
          {article.sections.map((section, index) => <a href={`#section-${index}`} key={section}>{index + 1}. {section}</a>)}
        </nav>
        ...
        <div className="article-disclosure">Affiliate disclosure: {site.affiliateNotice}</div>
      </article>
